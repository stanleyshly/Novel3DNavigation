...
class Solver():
	def __init__(self, data_loader, config):
		...
		from torchsummary import summary
		summary(self.model.cuda(), (3, 244,244))
		...
		for epoch in range(start_epoch, num_epochs):
			...
			for phase in ['train', 'val']:
				...
				for i, (inputs, poses) in enumerate(data_loader):
					...
					# forward
					#print( self.config.model == "Googlenet")
					if self.config.model == "Resnet":
						pos_out, ori_out, _ = self.model(inputs) #Resnet
					
					if self.config.model == "Googlenet":
						pos_out, ori_out = self.model(inputs) #Googlenet
					...
					'''  homoscedastic uncertainty based automatic weight scaling'''
					...
					''' legacy beta scaled loss'''
					#loss_pos = F.l1_loss(pos_out, pos_true) # keep these two lines commented?
